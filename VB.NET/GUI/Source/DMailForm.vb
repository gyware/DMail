...
Public Class DMailForm
    Private Sub RunBtn_Click(sender As Object, e As EventArgs) Handles RunBtn.Click
        ...
        Dim recipientsRaw As String = InputBox("To", "$safeprojectname$ (VB.NET)")
        Dim recipients As New MailAddressCollection()
        For Each recipientRaw As String In Split(recipientsRaw, ",")
            recipients.Add(New MailAddress(Trim(recipientRaw)))
        Next
        ...
        For Each recipient In recipients
            If customSMTP = Nothing Then
                Dim emailDomain As String = recipient.Host
                ...
                SMTP.Send(mail)
                MsgBox("Email sent successfully to """ & recipient.Address & """.", vbInformation + vbOKOnly, "VBMail")
            Else
