...
Do until success = True
	mail.Send
	If Err.Number <> 0 Then
		button = MsgBox("Error: mail send failed." & vbCrLf & "To: " & recipient & vbCrLf & "From: " & from & vbCrLf & "Description: " & Err.Description, vbCritical + vbAbortRetryIgnore + vbDefaultButton2 + vbSystemModal, "DMail (VBScript)")
		Select Case button
		Case vbAbort
			WScript.Quit
		Case vbRetry
...
mail.Configuration.Fields.Update
	...
	mail.Send
	If Err.Number <> 0 Then
		button = MsgBox("Error: Email send failed." & vbCrLf & "To: " & recipient & vbCrLf & "From: " & from & vbCrLf & "Description: " & Err.Description & vbCrLf & "Note: If you can't send any email to this address, and both the ""To"" and ""From"" addresses are valid, it is most likely the SMTP server is rejecting your email due to your IP address not being recognised by anti-spam software as an SMTP server. You can try again or alternatively get your IP address whitelisted by the recipient's SMTP server.", vbCritical + vbAbortRetryIgnore + vbDefaultButton2 + vbSystemModal, "DMail (VBScript)")
		Select Case button
		Case vbAbort
			WScript.Quit
		Case vbRetry
